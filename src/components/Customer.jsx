...
const Customer = ({data, onEdit }) => {
    ...
    <View className="block w-full rounded-lg bg-white text-left p-2 my-2">
        <View className="p-4">
            <Text className="block font-sans text-sm antialiased font-normal leading-normal text-gray-700 opacity-75">{data?.code || ""}</Text>
            <Text className="block font-sans text-base antialiased font-medium leading-relaxed text-blue-gray-900">{data?.name || ""}</Text>
            <Text className="block font-sans text-sm antialiased font-normal leading-normal text-gray-700 opacity-75">{data?.address || ""}</Text>
        </View>
